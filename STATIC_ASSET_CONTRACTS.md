...
## Closed-surface point classification

`axm_uc.triangle_queries.ClosedTriangleSurface(vertices, triangles)` provides a separate, discrete query for an already welded, non-self-intersecting closed triangle surface. `classify(point)` returns `inside`, `outside`, `boundary`, or `hold`. Coordinates and the default tolerance of 1e-7 are in metres. Invalid/open topology, degenerate triangles, nonfinite data and invalid indices raise `ValueError`. The two-triangles-per-edge check does not establish freedom from self-intersections; that remains a caller precondition. Boundary and hold are not strict penetration or clearance results.
