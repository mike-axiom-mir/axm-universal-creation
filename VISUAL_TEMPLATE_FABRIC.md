...
The public module remains `axm_uc.visual_templates`. The retained kernel is `visual_template_core.py`; explicit extension packs add game/product, creative/narrative, AXM-system, shared-game, key-art, card/deck and cinematic-overlay knowledge before the facade validates one composed v1 catalog. Sticker Fabric remains the immutable/versioned local registry bridge. Mathematical work can enrich declared ratios/ranges through `math_hooks` without replacing template identity.

## Current v7 candidate census

- **96 reusable visual/state primitives**
- **15 coherent style systems**
- **135 responsive screen archetypes**
- **13 whole-product archetypes**
- **148 exact screen/product definitions** when explicitly installed into Sticker Registry
...
`visual.cards.core` provides eleven source-first card/deck surfaces: project hub, face/back editing, artwork, text/stats, rules/abilities, rarity, finish/foil, deck construction, print sheets and review/export. Face/back geometry, art/crop, exact text/rules, costs, rarity cues, deck references, foil layers and print guides remain separate state.

### Editable cinematic titles / credits / trailer overlays

`visual.cinematic.core` provides eleven timed overlay surfaces:
...
Its `visual.cinematic.motion` style is a replaceable starting language. The structural contract is timing-aware editability:

- `title-card` keeps text, timing and layout separate;
- `lower-third` keeps identity and duration/transition timing explicit;
- `subtitle-cue` keeps exact text/speaker/timecode state;
- `credit-entry` preserves exact names and roles;
- `timeline-cue` uses explicit time ranges;
- `transition-safe-zone` forbids silent occlusion of required content;
- `overlay-track` retains order and timing;
- `shot-marker` pins exact beat/shot/chapter identity and time;
- `end-card` keeps content and duration explicit;
- `legal-line` preserves exact legal/disclaimer text.

Derived trailer/render/export output remains subordinate to richer editable timing source state.
...
- exact census: 15 styles / 96 primitives / 135 screens / 13 products;
- all 135 screens inside six representative viewport shapes;
- exact product screen order and flow across those viewports;
- parseable galleries for all prior proof products plus cinematic overlays;
- all 148 screen/product definitions install through the existing immutable Sticker Registry;
- title text/timing/layout separation, exact lower-third timing, subtitle timecodes, exact credits/legal text, explicit timeline ranges/track order, transition-safe required content, exact shot markers and explicit end-card duration remain present;
- prior source boundaries from cards, key art, comics, games and AXM remain intact;
- copy safety, strict variant rejection, invalid-geometry rejection and exact Sticker slot binding remain intact.

## Truth boundary

Current evidence proves deterministic structural knowledge and editability/state vocabulary only. It does **not** prove target-engine rendering, real motion quality, subtitle transcription accuracy, trailer editorial quality, transition rendering, platform compliance, gameplay feel, authoring ergonomics or aesthetic acceptance.
