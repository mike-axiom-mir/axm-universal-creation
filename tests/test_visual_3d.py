...
class Visual3DForgeTests(unittest.TestCase):
    ...
    def test_aaa_assessment_requires_technical_and_bound_visual_pass(self):
        ...
        manifest = {
            ...
            "render_proofs": [{"sha256": char * 64, "angle_degrees": angle}
                              for char, angle in zip("acde", (32, 122, 212, 302))],
        }
        ...
        single_view = assess_3d_output(receipt, manifest, {"artifact_sha256": proof_sha, "criteria": criteria})
        self.assertFalse(single_view["visual_pass"])
        review = {"views": [{"artifact_sha256": row["sha256"], "criteria": criteria}
                            for row in manifest["render_proofs"]]}
        accepted = assess_3d_output(receipt, manifest, review)
        self.assertEqual(accepted["status"], "AAA_ACCEPTED")
        review["views"][2]["criteria"] = {**criteria, "aaa-form-hierarchy": "FAIL"}
        self.assertFalse(assess_3d_output(receipt, manifest, review)["visual_pass"])
        manifest["render_proofs"][3] = manifest["render_proofs"][0]
        self.assertFalse(assess_3d_output(receipt, manifest)["technical_pass"])
