...
from .asset_geometry import REVIEW_SCHEMA, review_static_glb, validate_static_contract
...
ITERATION_RUN_SCHEMA = "axm.3d-iteration-run/v0.1"
ASSET_CATALOG = {
    "axiom-bastion-frame": {
        "faction": "axiom",
...
ITERATION_STAGES = (
    {
        "id": "silhouette",
        "criteria": ("faction-silhouette", "non-toy-proportions"),
        "constraint": "Resolve primary proportions, stance, negative space, and faction silhouette before secondary detail.",
    },
    {
        "id": "structure",
        "criteria": ("functional-topology-all-angles",),
        "constraint": "Resolve load paths, articulation, attachment roots, and coherent topology across every proof angle.",
    },
    {
        "id": "materials",
        "criteria": ("material-authenticity",),
        "constraint": "Resolve authored PBR separation, surface age, scale, and restrained emission at hero distance.",
    },
    {
        "id": "polish",
        "criteria": ("aaa-form-hierarchy",),
        "constraint": "Resolve large-medium-small form hierarchy, local manufacturing detail, and hero-camera finish.",
    },
    {
        "id": "engine-final",
        "criteria": tuple(AAA_QUALITY_GATES["required_visual_criteria"]),
        "constraint": "Reconfirm every visual criterion on the current artifact together with all engine export gates.",
    },
)
...
def catalog_3d() -> dict[str, Any]:
    return {
        ...
        "iteration_contract": {
            "schema": ITERATION_RUN_SCHEMA,
            "default_minimum_iterations": 7,
            "default_maximum_iterations": 24,
            "ordered_stages": [row["id"] for row in ITERATION_STAGES],
            "outcome_requires_all_stages": True,
            "outcome_requires_artifact_bound_aaa": True,
        },
        "truth": {
            ...
            "oneForgePassIsNeverAnIterationOutcome": True,
        },
    }
...
def assess_3d_output(receipt: dict[str, Any], manifest: dict[str, Any], visual_review: Any = None) -> dict[str, Any]:
    ...
    proof_hashes = [row.get("sha256", "") for row in render_proofs]
    angles = [row.get("angle_degrees") for row in render_proofs]
    distinct_proofs = (
        len(proof_hashes) >= AAA_QUALITY_GATES["minimum_render_angles"]
        and all(isinstance(value, str) and re.fullmatch(r"[0-9a-f]{64}", value) for value in proof_hashes)
        and len(set(proof_hashes)) == len(proof_hashes)
        and all(type(value) in (int, float) and 0 <= value < 360 for value in angles)
        and len(set(angles)) == len(angles)
    )
    if distinct_proofs:
        ordered_angles = sorted(angles)
        gaps = [right - left for left, right in zip(ordered_angles, ordered_angles[1:] + [ordered_angles[0] + 360])]
        distinct_proofs = max(gaps) <= 135
    gates = {
        ...
        "multi-angle-proof": distinct_proofs,
        "source-blend": bool(manifest.get("source", {}).get("sha256")),
        "named-exports": all(name in exports for name in ("lod0", "lod1", "lod2", "collision")),
    }
    spatial = receipt.get("spatial_contract_review")
    if spatial is not None:
        gates["static-spatial-contract"] = (
            isinstance(spatial, dict) and spatial.get("schema") == REVIEW_SCHEMA
            and spatial.get("status") == "PASS" and bool(spatial.get("measurements"))
            and bool(lod0.get("sha256")) and spatial.get("artifact_sha256") == lod0.get("sha256")
        )
    ...
    if visual_review is not None:
        ...
        # A legacy single-view review remains useful evidence, but cannot certify
        # the other sides of a 3D asset. Every current proof needs its own review.
        views = visual_review.get("views", [visual_review])
        if not isinstance(views, list) or any(not isinstance(row, dict) for row in views):
            raise TypeError("visual_review.views must be a list of review objects")
        if any(not isinstance(row.get("criteria", {}), dict) for row in views):
            raise TypeError("each visual review must contain a criteria object")
        reviewed_hashes = [str(row.get("artifact_sha256", "")) for row in views]
        required = AAA_QUALITY_GATES["required_visual_criteria"]
        bound = bool(distinct_proofs) and len(reviewed_hashes) == len(proof_hashes) and set(reviewed_hashes) == set(proof_hashes)
        criteria = {
            name: "PASS" if views and all(str(row.get("criteria", {}).get(name, "UNKNOWN")).upper() == "PASS" for row in views)
            else "FAIL" if any(str(row.get("criteria", {}).get(name, "UNKNOWN")).upper() == "FAIL" for row in views)
            else "UNKNOWN"
            for name in required
        }
        passed = bound and all(criteria[name] == "PASS" for name in required)
        visual = {
            "status": "PASS" if passed else "FAIL",
            "artifact_sha256": reviewed_hashes[0] if len(reviewed_hashes) == 1 else None,
            "reviewed_artifact_sha256s": reviewed_hashes,
...
def forge_3d_asset(
    ...
    *,
    ...
    spatial_contract: Any = None,
) -> dict[str, Any]:
    machine_root = Path(root).resolve()
    checked_contract = validate_static_contract(spatial_contract) if spatial_contract is not None else None
    ...
    if checked_contract is not None:
        receipt["spatial_contract_review"] = review_static_glb(target / manifest["exports"]["lod0"]["path"], checked_contract)
        (target / "static-contract-review.json").write_text(
            json.dumps(receipt["spatial_contract_review"], indent=2) + "\n", encoding="utf-8", newline="\n")
