...
def _rotation_delta_degrees(expected: list[float], observed: list[float]) -> float:
    ...
    for row in range(3):
        for column in range(3):
            trace += expected[row * 4 + column] * observed[row * 4 + column] if row == column else 0.0
    # Robust full R_expected^T * R_observed trace.
    trace = sum(
...
def _part(raw: Any, index: int) -> dict[str, Any]:
    ...
    return {
        "id": _identifier(raw["id"], f"parts[{index}].id"),
        "shape": shape,
...
def _gauge(raw: Any, index: int, part_ids: set[str]) -> dict[str, Any]:
    ...
    if kind == "distance":
        expected = {"id", "type", "a", "b", "target", "tolerance"}
        if set(raw) != expected:
    ...
    if kind in {"alignment", "spacing"}:
        ...
        return result
    if kind == "angle":
        if set(raw) != {"id", "type", "a", "a_axis", "b", "b_axis", "target_degrees", "tolerance_degrees"}:
    ...
    if kind == "orientation":
        if set(raw) != {"id", "type", "part", "local_axis", "world_axis", "target_degrees", "tolerance_degrees"}:
...
def validate_sketch(raw: Any) -> dict[str, Any]:
    ...
    if not isinstance(raw, dict) or set(raw) != required or raw.get("schema") != SKETCH_SCHEMA:
    ...
    gauge_ids = [item["id"] for item in gauges]
    ...
    result = {
        ...
        "id": _identifier(raw["id"], "sketch.id"),
        "purpose": _text(raw["purpose"], "sketch.purpose", 2000),
        "units": _identifier(raw["units"], "sketch.units"),
        "parts": parts,
        "gauges": gauges,
        "tolerances": {
            "position": _number(tolerances["position"], "tolerances.position", 0),
    ...
    result["sketch_digest"] = _digest(result)
    return result
...
def _gauge_result(gauge: dict[str, Any], observed: dict[str, dict[str, Any]]) -> dict[str, Any]:
    needed = []
    if gauge["type"] in {"distance", "angle"}: needed = [gauge["a"], gauge["b"]]
    elif gauge["type"] in {"alignment", "spacing"}: needed = gauge["parts"]
    elif gauge["type"] in {"orientation", "size"}: needed = [gauge["part"]]
    elif gauge["type"] == "symmetry": needed = [value for pair in gauge["pairs"] for value in (pair["a"], pair["b"])]
    missing = sorted(set(needed) - set(observed))
    if missing:
        return {"id": gauge["id"], "type": gauge["type"], "status": "HOLD", "missing_parts": missing}
    kind = gauge["type"]
    residual = 0.0
    measured: Any = None
    tolerance = gauge.get("tolerance", gauge.get("tolerance_degrees", 0.0))
    if kind == "distance":
        measured = _distance(_position(observed[gauge["a"]]["frame"]), _position(observed[gauge["b"]]["frame"]))
        residual = abs(measured - gauge["target"])
    elif kind == "alignment":
        axis_index = {"x": 0, "y": 1, "z": 2}[gauge["axis"]]
        first = _position(observed[gauge["parts"][0]]["frame"])
        residual = max(math.sqrt(sum((position[index] - first[index]) ** 2 for index in range(3) if index != axis_index))
                       for position in (_position(observed[part]["frame"]) for part in gauge["parts"]))
        measured = residual
    elif kind == "spacing":
        axis_index = {"x": 0, "y": 1, "z": 2}[gauge["axis"]]
        values = [_position(observed[part]["frame"])[axis_index] for part in gauge["parts"]]
        ...
        measured = _angle(_axis_vector(observed[gauge["a"]]["frame"], gauge["a_axis"]),
                          _axis_vector(observed[gauge["b"]]["frame"], gauge["b_axis"]))
        residual = abs(measured - gauge["target_degrees"])
    elif kind == "orientation":
        measured = _angle(_axis_vector(observed[gauge["part"]]["frame"], gauge["local_axis"]), AXES[gauge["world_axis"]])
        residual = abs(measured - gauge["target_degrees"])
    elif kind == "symmetry":
        axis_index = {"x": 0, "y": 1, "z": 2}[gauge["axis"]]
        pair_residuals = []
        for pair in gauge["pairs"]:
            first = _position(observed[pair["a"]]["frame"])
            second = list(_position(observed[pair["b"]]["frame"]))
            second[axis_index] = 2.0 * gauge["center"] - second[axis_index]
            pair_residuals.append(_distance(first, tuple(second)))
        residual = max(pair_residuals)
        measured = pair_residuals
    elif kind == "size":
        actual = observed[gauge["part"]]["size"]
        residuals = [abs(actual[index] - gauge["target"][index]) for index in range(3)]
        residual = max(residuals); measured = actual
...
def compare_sketch(sketch_raw: Any, observation_raw: Any) -> dict[str, Any]:
    ...
    observation = validate_observation(observation_raw, sketch)
    planned = {item["id"]: item for item in sketch["parts"]}
    observed = {item["id"]: item for item in observation["parts"]}
    part_results = []
    for part_id, expected in planned.items():
        actual = observed.get(part_id)
        if actual is None:
            part_results.append({"id": part_id, "status": "HOLD", "reason": "part not observed"})
            continue
        ...
        orientation_residual = _rotation_delta_degrees(expected["frame"], actual["frame"])
        failures = []
        ...
        part_results.append({"id": part_id, "status": "PASS" if not failures else "FAIL",
                             "position_residual": round(position_residual, 12),
                             "size_residual": round(size_residual, 12),
                             "orientation_residual_degrees": round(orientation_residual, 12),
                             "failed_dimensions": failures})
    gauge_results = [_gauge_result(gauge, observed) for gauge in sketch["gauges"]]
    statuses = [row["status"] for row in part_results + gauge_results]
    status = "FAIL" if "FAIL" in statuses else "HOLD" if "HOLD" in statuses else "PASS"
    report = {"schema": REPORT_SCHEMA, "truth_status": "DETERMINISTIC_SKETCH_VS_OBSERVED_STATE_MEASUREMENT",
              "status": status, "passed": status == "PASS", "sketch_digest": sketch["sketch_digest"],
              "observer": copy.deepcopy(observation["observer"]), "parts": part_results, "gauges": gauge_results,
              "repair_direction": [row["id"] for row in part_results + gauge_results if row["status"] != "PASS"],
              "limitations": [
                  "measurements compare caller-supplied rigid frames and sizes only",
                  "PASS does not prove mesh fit, collision clearance, strength, manufacturability, physics, semantics, aesthetics, or target-engine quality",
                  "schematic sketch intent is not an automatic design-quality verdict",
              ]}
...
def _svg(sketch: dict[str, Any]) -> str:
    ...
    chunks = ['<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="420" viewBox="0 0 1200 420">',
              '<rect width="1200" height="420" fill="white"/>',
              '<text x="20" y="24" font-family="monospace" font-size="14">AXM schematic blockout — center/size projection, not a render</text>']
    panel_width = 390.0
    for view_index, (title, first_axis, second_axis) in enumerate(views):
        x_offset = 10.0 + view_index * 400.0
        bounds = []
        for part in parts:
            position = _position(part["frame"]); size = part["size"]
            bounds.append((position[first_axis] - size[first_axis] / 2, position[first_axis] + size[first_axis] / 2,
                           position[second_axis] - size[second_axis] / 2, position[second_axis] + size[second_axis] / 2))
        min_x = min(value[0] for value in bounds); max_x = max(value[1] for value in bounds)
        min_y = min(value[2] for value in bounds); max_y = max(value[3] for value in bounds)
        span_x = max(max_x - min_x, 1e-9); span_y = max(max_y - min_y, 1e-9)
        scale = min(340.0 / span_x, 300.0 / span_y)
        chunks.append(f'<rect x="{x_offset}" y="40" width="{panel_width}" height="360" fill="none" stroke="black"/>')
        chunks.append(f'<text x="{x_offset + 8}" y="60" font-family="monospace" font-size="13">{html.escape(title)}</text>')
        for part, bound in zip(parts, bounds):
            x = x_offset + 25 + (bound[0] - min_x) * scale
            y = 370 - (bound[3] - min_y) * scale
            width = max((bound[1] - bound[0]) * scale, 2)
            height = max((bound[3] - bound[2]) * scale, 2)
            chunks.append(f'<rect x="{x:.3f}" y="{y:.3f}" width="{width:.3f}" height="{height:.3f}" fill="none" stroke="black"/>')
            chunks.append(f'<text x="{x + 3:.3f}" y="{y + 13:.3f}" font-family="monospace" font-size="10">{html.escape(part["id"])}</text>')
    chunks.append('<text x="20" y="414" font-family="monospace" font-size="11">Rotated extents are not rasterized; exact rigid frames remain in sketch.json.</text>')
    chunks.append('</svg>')
    return "\n".join(chunks) + "\n"
...
def _resolve_path(root: Path, requested: str) -> Path:
    ...
    return (Path(root) / value).resolve() if not value.is_absolute() else value.resolve()
...
def materialize_sketch(root: Path, path: str, sketch_raw: Any, *, replace: bool = False) -> dict[str, Any]:
    sketch = validate_sketch(sketch_raw)
    target = _resolve_path(root, _text(path, "path", 4000))
    if _machine_body(root, target):
        raise DesignWorkshopError("design sketch materialization cannot rewrite the live machine body")
    files = {"sketch.json": json.dumps(sketch, ensure_ascii=False, indent=2, sort_keys=True) + "\n", "sketch.svg": _svg(sketch)}
    try:
        result = build_project(target=target, files=files, project_type="generic",
                               checks=[{"type": "file-set", "files": sorted(files), "mode": "exact"},
                                       {"type": "json-valid", "path": "sketch.json"}],
                               replace=replace, publish_mode="validated")
    except ProjectError as exc:
        raise DesignWorkshopError(str(exc), exc.details) from exc
    result["truth_status"] = "VALIDATED_DESIGN_SKETCH_DESCRIPTOR_PROJECT"
    result["sketch_digest"] = sketch["sketch_digest"]
    return result
...
def operate_design_workshop(root: Path, inputs: dict[str, Any]) -> dict[str, Any]:
    ...
    if operation == "inspect-workshop":
        return {"truth_status": "DECLARED_DESIGN_WORKSHOP_V0_1", **design_workshop_summary()}
    if operation == "validate-sketch":
        return {"truth_status": "DETERMINISTIC_DESIGN_SKETCH_VALIDATION", "sketch": validate_sketch(inputs.get("sketch"))}
    if operation in {"compare-sketch", "measure-design"}:
        return compare_sketch(inputs.get("sketch"), inputs.get("observation"))
    if operation == "materialize-sketch":
        replace = inputs.get("replace", False)
        if not isinstance(replace, bool):
            raise DesignWorkshopError("replace must be boolean")
