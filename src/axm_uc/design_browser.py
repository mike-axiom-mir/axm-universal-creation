...
BROWSER_CAPTURE_SCHEMA = "axm.design-browser-capture-receipt/v0.1"
...
def _offline_browser_command(browser: Path, viewport: dict[str, Any], screenshot: Path, target: Path) -> list[str]:
    return [
        str(browser),
        "--headless=new",
...
def capture_local_browser(
    ...
    with tempfile.TemporaryDirectory(prefix=".axm-design-browser-", dir=staging_parent) as temporary:
        ...
        for viewport in viewports:
            ...
            command = _offline_browser_command(browser, viewport, screenshot, target)
            try:
                completed = subprocess.run(
                    command,
                    cwd=target.parent,
                ...
                raise DesignBrowserError(
                    ...
                    {"viewport": viewport_id, "timeout_seconds": timeout},
                ) from exc
            except OSError as exc:
                raise DesignBrowserError(
                    "browser capture could not start",
                    {"viewport": viewport_id, "reason": str(exc)},
                ) from exc
        ...
        observation = record_render_observation(
            plan["plan_digest"],
            {
                ...
                "basis": "explicit local browser executable invoked in headless mode with external host resolution blocked",
            },
            captures,
        )
        observation["truth_status"] = "OBSERVED_LOCAL_BROWSER_ARTIFACT_BYTES"
        ...
        receipt = {
            ...
            "truth_status": "LOCAL_HEADLESS_BROWSER_CAPTURE_COMPLETED",
...
def design_browser_summary() -> dict[str, Any]:
    return {
        ...
        "computed_runtime_measurements_available": False,
