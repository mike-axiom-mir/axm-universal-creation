...
def _eval(expr: Any, values: dict[str, float], *, depth: int = 0) -> float:
    ...
    if name == "pow":
        if len(vals) != 2:
            raise ValueError("pow requires two args")
        return _finite(vals[0] ** vals[1])
    if name == "min":
        return min(vals)
    if name == "max":
    ...
    if name in {"sin", "cos", "tan"}:
        if len(vals) != 1:
            raise ValueError(f"{name} requires one arg")
...
def _constraint(spec: Any, values: dict[str, float]) -> dict[str, Any]:
    ...
    passed = {
        ...
        "le": left <= right + tolerance,
        "eq": abs(left - right) <= tolerance,
        "ge": left + tolerance >= right,
        "gt": left > right + tolerance,
    }[op]
    return {"label": spec["label"], "op": op, "left": left, "right": right,
...
def validate_family(family: Any) -> dict[str, Any]:
    ...
    for name, spec in parameters.items():
        ...
        base[name] = _finite(checked["default"])
...
def resolve_family(
    ...
    *,
    ...
    return {
        ...
        "constraints": checks,
        "truth_boundary": (
            "Numeric derivation and declared constraints were evaluated. Units are labels in v1; "
            "dimensional consistency, physical realism, manufacturability, rendered quality and "
            "real-world safety are not inferred."
        ),
    }
