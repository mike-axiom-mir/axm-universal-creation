...
const SELECTION_OP=['expand-faces','shrink-faces','invert-faces','faces-to-vertices','invert-vertices'];
const FACE_EDIT=['delete','flip','extract','duplicate','extrude-region','extrude-individual','inset-individual'];
...
function invoke(id,args){const hand=BY_ID.get(String(id||''));U.ensure(hand,'unknown mesh edit hand: '+id);args=args||{};let r;
  if(hand.family==='mesh-face-select'){
    ...
    if(hand.operation==='translate')r=E.translateVertices(args.mesh,args.selection,args.vector);else if(hand.operation==='scale')r=E.scaleVertices(args.mesh,args.selection,args.vector==null?args.factor:args.vector);else if(hand.operation.startsWith('rotate-'))r=E.rotateVertices(args.mesh,args.selection,hand.operation.slice(-1),args.degrees);else if(hand.operation==='smooth')r=E.smoothVertices(args.mesh,args.selection,args.spec);else r=E.noiseVertices(args.mesh,args.selection,args.spec);
  } else if(hand.family==='mesh-edge-select'){
    if(hand.operation==='all')r=E.edgesAll(args.mesh);else if(hand.operation==='boundary')r=E.edgesBoundary(args.mesh);else if(hand.operation==='by-angle')r=E.edgesByAngle(args.mesh,args.spec);else if(hand.operation==='by-length')r=E.edgesByLength(args.mesh,args.spec);else r=E.regionBoundaryEdges(args.mesh,args.selection);
