...
const audit=Hands.audit();assert.equal(audit.total,48);assert.equal(audit.by_family['rig-skeleton'],9);assert.equal(audit.by_family['rig-pose'],11);assert.equal(audit.by_family['rig-skin'],8);assert.equal(audit.by_family['animation-blendshape'],6);assert.equal(audit.by_family['animation-clip'],13);assert.equal(audit.by_family['rig-ik'],1);assert.equal(Platform.creativeHands.audit().total,388);assert.equal(Platform.creativeHands.recipeRegistry().count,395);assert.equal(Flow.summary().public_hands,388);assert.equal(Flow.summary().callable_recipes,395);
