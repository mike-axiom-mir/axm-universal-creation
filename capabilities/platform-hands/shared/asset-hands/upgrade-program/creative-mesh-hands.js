...
const BY_ID=new Map(DESCRIPTORS.map((d)=>[d.id,d]));function list(){return DESCRIPTORS.map((d)=>JSON.parse(JSON.stringify(d)));}function get(id){const d=BY_ID.get(String(id||''));return d?JSON.parse(JSON.stringify(d)):null;}function wrap(hand,result){const value={schema:RESULT_SCHEMA,version:'1.0.0',status:'EXECUTED',hand_id:hand.id,hand_digest:hand.digest,result};value.digest=U.sha256(value);return value;}
function invoke(id,args){const hand=BY_ID.get(String(id||''));U.ensure(hand,'unknown creative mesh hand: '+id);args=args||{};let result;
  if(hand.family==='mesh-primitive')result=Mesh.primitive(hand.operation,args.spec||args);
    ...
    else if(hand.operation==='scale')result=Mesh.scale(args.mesh,args.vector==null?args.factor:args.vector);
    ...
    const map={'recalc-normals':'recalcNormals','flip-winding':'flipWinding','center-origin':'centerOrigin','normalize-scale':'normalizeScale','remove-degenerate':'removeDegenerate'};
    if(hand.operation==='compact')result=Mesh.compact(args.mesh);
    ...
    else result=Mesh[map[hand.operation]](args.mesh,args.target==null?(args.epsilon==null?undefined:args.epsilon):args.target);
