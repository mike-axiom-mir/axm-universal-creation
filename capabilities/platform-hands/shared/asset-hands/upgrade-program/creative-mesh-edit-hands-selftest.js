...
assert.equal(Platform.creativeHands.audit().total,279);
assert.equal(Platform.creativeHands.recipeRegistry().count,286);

const plane=MeshHands.invoke('creative.mesh-primitive.plane',{spec:{id:'plane',detail:4}}).result;
...
const faceVertices=EditHands.invoke('creative.mesh-selection.faces-to-vertices',{mesh:plane,selection:indexed}).result;assert(faceVertices.vertices.length>=4);
...
const deleted=EditHands.invoke('creative.mesh-face-edit.delete',{mesh:plane,selection:EditHands.invoke('creative.mesh-face-select.indices',{mesh:plane,faces:[0]}).result}).result;assert.equal(deleted.indices.length/3,31);
const flipped=EditHands.invoke('creative.mesh-face-edit.flip',{mesh:plane,selection:EditHands.invoke('creative.mesh-face-select.indices',{mesh:plane,faces:[0]}).result}).result;assert.equal(flipped.indices[1],plane.indices[2]);assert.equal(flipped.indices[2],plane.indices[1]);
...
const duplicated=EditHands.invoke('creative.mesh-face-edit.duplicate',{mesh:plane,selection:EditHands.invoke('creative.mesh-face-select.indices',{mesh:plane,faces:[0]}).result,spec:{offset:[0,1,0]}}).result;assert.equal(duplicated.indices.length/3,33);
...
console.log(JSON.stringify({status:'PASS',edit_hands:audit.total,public_hands:Platform.creativeHands.audit().total,recipes:Platform.creativeHands.recipeRegistry().count,plane:plane.digest,extrude:regionExtrude.digest,split:split.digest,sharp_edges:sharp.edges.length},null,2));