...
const registries=Object.freeze([CoreHands,MeshHands,MeshEditHands,MeshAdvancedHands,MeshModelingHands]);
const recipeRegistries=Object.freeze([MeshHands,MeshEditHands,MeshAdvancedHands,MeshModelingHands]);
...
function recipeRegistry(){const base=Recipes.registry(),extra=additionalRecipes(),value={schema:base.schema,version:'6.0.0',principle:base.principle,tools:base.tools.concat(extra),count:base.count+extra.length,executable_hands:audit().total};value.digest=U.sha256(value);return value;}
function invokeRecipe(id,args){for(const registry of recipeRegistries){const hand=registry.list().find((h)=>recipeId(h)===id);if(hand){const result=registry.invoke(hand.id,args),value={schema:'axm.creative-tool-recipe/v1',version:'6.0.0',tool_id:id,state:'EXECUTABLE',primitive:hand.id,result};value.digest=U.sha256(value);return value;}}return Recipes.invoke(id,args);}
module.exports=Object.freeze({version:'5.0.0',list:allHands,get,invoke,audit,recipeRegistry,invokeRecipe});