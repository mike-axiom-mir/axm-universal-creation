...
function validate(world, constraints, options) {
  ...
  return {
    ...
    warnings: (composer.warnings || []).concat([
      'Collision isolation is component-wide across translation mounts, distance joints, distance limits and axis locks: every body in one connected constraint component temporarily shares one negative collision group during the donor-core collision stage.',
...
function step(world, constraints, dt, options) {
  ...
  return {
    ...
    evidence: [
      prepared.components.length + ' connected constraint component(s) discovered across four supported translation-only constraint families in deterministic body-id order',
    ...
    limitations: [
      ...
      'Translation mounts, center-to-center distance joints, center-distance limits and world-axis translation locks are understood; no additional joint families are implied.',
