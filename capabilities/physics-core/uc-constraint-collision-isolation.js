...
function collisionGroup(item) {
  return item && item.collision && Number.isFinite(Number(item.collision.group))
    ? Math.round(Number(item.collision.group))
    : 0;
}
...
function connectedComponents(normalized) {
  ...
  Array.from(adjacency.keys()).sort().forEach(start => {
    ...
    members.sort();
    if (members.length > 1) components.push(members);
  });
...
function prepare(world, constraints, options) {
  ...
  components.forEach((memberIds, index) => {
    ...
    if (originalGroups.some(group => group !== 0)) {
      receipt.reason = 'EXISTING_GROUP_SEMANTICS';
      receipts.push(receipt);
      return;
    }
    ...
    if (group == null) {
      receipt.reason = 'TEMP_GROUP_CAPACITY_EXHAUSTED';
      receipts.push(receipt);
      return;
    }
    ...
    receipt.temporaryGroup = group;
    receipt.reason = 'APPLIED_COMPONENT_NEGATIVE_GROUP';
    receipts.push(receipt);
  });
...
function restoreGroups(world, receipts) {
  ...
  receipts.forEach(receipt => {
    ...
    receipt.originalGroups.forEach(entry => {
      const item = body(out, entry.id);
      if (item && item.collision) item.collision.group = entry.group;
    });
  });
  return out;
}
...
function refreshDiagnostics(world, prior) {
  ...
  const refreshed = Core.measure(
    world,
    prior.substeps || 0,
    prior.maxPenetration || 0,
    prior.broadphasePairs || 0,
    {
...
function validate(world, constraints, options) {
  ...
  if (composer.ok) {
    try {
      prepared = prepare(world, constraints || {}, options || {});
    } catch (error) {
...
function step(world, constraints, dt, options) {
  ...
  return {
    ...
    isolationDiagnostics: {
      ...
      appliedComponents: prepared.appliedComponents,
      skippedComponents: prepared.skippedComponents,
      isolatedBodies: prepared.isolatedBodies,
      receipts: clone(prepared.receipts),
      groupsRestored: true,
      contactEvidenceBasis: finalDiagnostics.contactEvidenceBasis
    },
    composerDiagnostics: clone(composed.composerDiagnostics),
    core: {
      diagnostics: clone(composed.core.diagnostics),
      events: clone(composed.core.events),
...
function simulate(world, constraints, steps, dt, options) {
  ...
  return {
    ...
    evidence: (last ? last.evidence : []).concat([count + ' collision-isolated composed step(s) completed'])
  };
}
