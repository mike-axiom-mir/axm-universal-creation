...
function constraintEdges(normalized) {
  ...
  (normalized.directionLimits || []).forEach(item => edges.push({ family: 'direction-limits', id: item.id, a: item.a, b: item.b }));
...
function validate(world, constraints, options) {
  ...
  return {
    ...
    warnings: (composer.warnings || []).concat([
      'Collision isolation is component-wide across translation mounts, distance joints, distance limits, axis locks, axis limits, fixed-direction locks and fixed-direction limits: every body in one connected constraint component temporarily shares one negative collision group during the donor-core collision stage.',
...
function step(world, constraints, dt, options) {
  ...
  return {
    ...
    evidence: [
      prepared.components.length + ' connected constraint component(s) discovered across seven supported translation-only constraint families in deterministic body-id order',
    ...
    limitations: [
      ...
      'Translation mounts, center-to-center distance joints, center-distance limits, world-axis translation locks, world-axis translation limits, fixed world-space direction locks and fixed world-space direction limits are understood; no additional joint families are implied.',
      ...
      'Direction-limit edges participate in component topology even while their allowed projected interval is physically slack and perpendicular translation remains free.',
