...
assert.equal(Core.checksum(replayA.world), Core.checksum(replayB.world), 'four-family guarded axis-lock path must replay deterministically in one JS runtime');
assert.match(enabled.limitations.join(' '), /not scientific validation/i);

console.log('UC Axis Lock Integration selftest: PASS (single-step composition, activity filtering, component isolation, orthogonal freedom and deterministic replay)');
