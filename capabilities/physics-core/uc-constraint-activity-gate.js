...
const FAMILY_KEYS = Object.freeze(['mounts', 'distanceJoints', 'distanceLimits']);
...
function partition(normalized) {
  const active = { mounts: [], distanceJoints: [], distanceLimits: [] };
  const disabled = { mounts: [], distanceJoints: [], distanceLimits: [] };
...
function prepare(world, constraints) {
  ...
  return {
    ...
    disabled: partitioned.disabled,
    diagnostics: familyDiagnostics(partitioned)
  };
}
...
function validate(world, constraints) {
  ...
  if (composer.ok) {
    try {
      prepared = prepare(world, constraints || {});
    } catch (error) {
  ...
  return {
    ...
    warnings: (composer.warnings || []).concat([
      'Disabled constraints are still normalized and validated for source/body-reference integrity, but this gate excludes them from solver residuals and collision-isolation topology before delegation.',
...
function step(world, constraints, dt, options) {
  ...
  return {
    ...
    limitations: [
      ...
      'The gate does not add angular inertia, rotating anchors, hinge, slider, rotational weld, motor or gear semantics.',
