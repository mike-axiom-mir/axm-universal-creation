...
const VERSION = '0.1.0';
const LIMIT_SCHEMA = 'axm.uc-distance-limit/v0.1';
const STEP_SCHEMA = 'axm.uc-distance-limit-step/v0.1';
const SIMULATION_SCHEMA = 'axm.uc-distance-limit-simulation/v0.1';
...
function normalizeLimit(input, world, index) {
  ...
  if (!a || !b || a === b) throw new Error('distance limit requires distinct a and b body ids');
  const bodyA = body(world, a);
  const bodyB = body(world, b);
  if (!bodyA || !bodyB) throw new Error('distance limit body not found: ' + (!bodyA ? a : b));

  const hasMin = input.minLength !== undefined && input.minLength !== null;
  const hasMax = input.maxLength !== undefined && input.maxLength !== null;
  if (!hasMin && !hasMax) throw new Error('distance limit requires minLength and/or maxLength');

  const minLength = hasMin ? bounded(input.minLength, 0, MAX_LENGTH, 0) : 0;
  const maxLength = hasMax ? bounded(input.maxLength, 0, MAX_LENGTH, MAX_LENGTH) : MAX_LENGTH;
  if (minLength > maxLength) throw new Error('distance limit minLength must be <= maxLength');

  return {
    schema: LIMIT_SCHEMA,
    id: String(input.id || ('distance-limit-' + String(index + 1).padStart(3, '0'))),
    a,
    b,
    minLength,
    maxLength,
    ...
    maxCorrection: bounded(input.maxCorrection, 0, MAX_LENGTH, 1e6),
...
function normalizeLimits(limits, world) {
  ...
  if (list.length > MAX_LIMITS) throw new Error('distance limit count exceeded');
  const normalized = list.map((limit, index) => normalizeLimit(limit, world, index));
  const seen = new Set();
  normalized.forEach(limit => {
    if (seen.has(limit.id)) throw new Error('duplicate distance limit id ' + limit.id);
    seen.add(limit.id);
  });
  return normalized.sort((a, b) => a.id.localeCompare(b.id));
}

function positionError(distance, limit) {
  if (distance < limit.minLength) return distance - limit.minLength;
  if (distance > limit.maxLength) return distance - limit.maxLength;
  return 0;
}

function limitState(distance, limit) {
  if (distance < limit.minLength) return 'BELOW_MIN';
  if (distance > limit.maxLength) return 'ABOVE_MAX';
  if (Math.abs(distance - limit.minLength) <= limit.slop) return 'AT_MIN';
  if (Math.abs(distance - limit.maxLength) <= limit.slop) return 'AT_MAX';
  return 'SLACK';
}
...
function measureLimit(world, limit) {
  ...
  if (!a || !b) throw new Error('distance limit body missing for ' + limit.id);
  const distance = Math.hypot(b.position.x - a.position.x, b.position.y - a.position.y);
  const error = positionError(distance, limit);
  return {
    id: limit.id,
    a: limit.a,
    b: limit.b,
    ...
    error: round(error),
    state: limitState(distance, limit),
...
function velocityViolation(world, limit) {
  ...
  const relative = (b.velocity.x - a.velocity.x) * axis.x + (b.velocity.y - a.velocity.y) * axis.y;
  const atMin = axis.distance <= limit.minLength + limit.slop;
  const atMax = axis.distance >= limit.maxLength - limit.slop;
...
function solvePosition(world, limit) {
  ...
  const axis = normalizeAxis({ x: b.position.x - a.position.x, y: b.position.y - a.position.y });
  const errorBefore = positionError(axis.distance, limit);
  ...
  const afterDistance = Math.hypot(b.position.x - a.position.x, b.position.y - a.position.y);
  const errorAfter = positionError(afterDistance, limit);
  return {
    id: limit.id,
    enabled: true,
    solved: true,
...
function refreshDiagnostics(world, coreDiagnostics, beforeCoreDiagnostics) {
  ...
  refreshed.distanceLimitPostStabilized = true;
  refreshed.contactEvidenceBasis = 'CORE_STAGE_BEFORE_POST_DISTANCE_LIMIT_STABILIZATION';
...
function step(world, limits, dt, options) {
  ...
  return {
    ...
    core: {
      ...
      worldBeforePostDistanceLimitStabilization: clone(coreStep.world)
    },
    evidence: [
      prepared.limits.length + ' center-distance limit(s) normalized in deterministic id order',
      prepared.positionIterations + ' bounded position iteration(s) executed before the donor-core step',
      prepared.velocityIterations + ' boundary-directed velocity iteration(s) executed before the donor-core step',
      'AXM Physics Core v' + Core.VERSION + ' executed collisions and integration from the limited start state',
      postPositionIterations + ' bounded position stabilization iteration(s) executed after the donor-core step',
      postVelocityIterations + ' boundary-directed velocity stabilization iteration(s) executed after the donor-core step',
      'Maximum enabled distance-limit violation changed from ' + maxViolationAfterCoreStep + ' after core integration to ' + maxViolationAfterStabilization + ' after post-core stabilization',
      'Final state checksum ' + Core.checksum(postWorld)
    ],
    limitations: [
      'Distance limits constrain body centers only; local anchors and angular response are not implemented.',
      'Slack range motion is intentionally unconstrained until a configured minimum or maximum boundary is reached.',
      'This capability is not yet a family inside the mixed constraint composer or component collision-isolation wrapper; callers must not chain independent step wrappers when they intend one shared donor-core integration step.',
      'Post-core projection can change positions after collision/contact evidence was generated; returned core events and contact geometry therefore describe the core stage before post-distance-limit stabilization.',
...
function simulate(world, limits, steps, dt, options) {
  ...
  return {
    ...
    limitDiagnostics: last ? last.limitDiagnostics : null,
    checksum: Core.checksum(out),
    evidence: (last ? last.evidence : []).concat([count + ' distance-limit step(s) completed'])
  };
}
...
function validate(world, limits) {
  ...
  return {
    ...
    warnings: [
      'Distance limits are translation-only center-to-center minimum/maximum constraints, not angular or anchored rigid joints.',
      'Range interiors are slack by design; velocity is blocked only when motion would cross an active boundary.',
      'The capability is additive around the preserved v0.3.1 donor core and is not yet integrated into the mixed constraint composer.',
      'Post-core stabilization is explicit and core-stage contact evidence is not silently relabeled as post-stabilization contact truth.'
    ]
  };
}
