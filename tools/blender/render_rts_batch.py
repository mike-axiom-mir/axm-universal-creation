...
def render(root,resolution=1400):
    ...
    for family,ids in families.items():
        ...
        for i,key in enumerate(ids):
            ...
            x=(i%cols-(cols-1)/2)*3.2;y=((rows-1)/2-i//cols)*3.8
            ...
            ob=bpy.data.objects.new('REVIEW label',curve);bpy.context.collection.objects.link(ob);ob.location=(x,y-1.55,.02)
        cam=setup_render(resolution,24);cam.location=(0,-21,14);point(cam,(0,0,.3));cam.data.ortho_scale=max(cols*3.35,rows*3.7)+1
