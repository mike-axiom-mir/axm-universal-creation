...
def armored_cowl(name: str, location: tuple[float, float, float], dimensions: tuple[float, float, float],
                  mat: bpy.types.Material, *, rotation: tuple[float, float, float] = (0, 0, 0),
                  bevel: float = .035) -> bpy.types.Object:
    """Tapered five-ring hard-surface volume for articulated primary masses.

    The forge previously built most anatomy from chamfered rectangular shells.
    This cowl introduces a controlled oval-octagonal section, tapered joint
    interfaces, and a crowned mid-volume without falling back to spheres.
    """
    hx, hy, hz = (value / 2 for value in dimensions)
    ring_specs = (
        (-hz, .54, .58),
    ...
    for z, sx, sy in ring_specs:
        for segment in range(8):
            angle = math.tau * segment / 8 + math.pi / 8
            vertices.append((math.cos(angle) * hx * sx, math.sin(angle) * hy * sy, z))
    faces: list[tuple[int, ...]] = [tuple(reversed(range(8))), tuple(range(32, 40))]
    for ring in range(4):
        base = ring * 8
        faces += [(base + i, base + (i + 1) % 8,
                   base + 8 + (i + 1) % 8, base + 8 + i) for i in range(8)]
    mesh = bpy.data.meshes.new(f"{name}_Mesh")
    mesh.from_pydata(vertices, [], faces)
    mesh.update()
    obj = bpy.data.objects.new(name, mesh)
...
def add_axiom_hero_v4(objects: list[bpy.types.Object], mats: dict[str, bpy.types.Material], rng: random.Random) -> None:
    ...
    def limb_shell(name: str, center: tuple[float, float, float], dims: tuple[float, float, float], side: int) -> None:
        ...
        objects.append(armored_cowl(f"{name}_PRIMARY_COWL", (x, y - dy * .12, z + dz * .01),
                                    (dx * .96, dy * .78, dz * .90), pale,
                                    rotation=(math.radians(2), side * math.radians(5), side * math.radians(2)), bevel=.036))
        objects.append(sculpted_shell(f"{name}_FRONT_SHELL", (x, y - dy * .50, z + dz * .02),
                                     (dx * .62, dy * .12, dz * .48), gm, front_scale=(.48, .58),
                                     rotation=(math.radians(3), side * math.radians(6), side * math.radians(2)), bevel=.022))
    ...
    for side in (-1, 1):
        ...
        objects.append(armored_cowl(f"AX4_CHEST_LOAD_{side}", (side * .55, -.28, 4.07), (1.02, .62, 1.10), gm,
                                    rotation=(math.radians(3), side * math.radians(11), side * math.radians(5)), bevel=.046))
    ...
    for side in (-1, 1):
        ...
        objects.append(armored_cowl(f"AX4_{p}_SHOULDER_COWL", (side * 1.66, -.08, 4.43),
                                    (.94, .82, .82), gm,
                                    rotation=(0, side * math.radians(10), side * math.radians(7)), bevel=.040))
    ...
    for side in (-1, 1):
        ...
        objects.append(armored_cowl(f"AX4_BACK_SHELL_{side}", (side * .69, .72, 4.13), (.48, .24, .68), pale,
                                    rotation=(math.pi, side * math.radians(7), 0), bevel=.026))
    ...
    # A protected rear pressure cowl establishes the same pale primary armor
    # hierarchy as the front. Service fins and conduits remain visible only at
    # its perimeter and terminate around one recessed central exhaust.
    objects.append(armored_cowl("AX4_BACK_MAIN_COWL", (0, 1.06, 4.22), (1.18, .26, 1.02), pale,
                                rotation=(math.pi, 0, 0), bevel=.038))
    objects.append(sculpted_shell("AX4_BACK_MAIN_VENT", (0, 1.22, 4.24), (.34, .08, .58), dark,
                                  front_scale=(.42, .56), rotation=(math.pi, 0, 0), bevel=.018))
    for slot in range(4):
        objects.append(box(f"AX4_BACK_MAIN_VENT_SLOT_{slot}",
                           (0, 1.275, 4.06 + slot * .12), (.19, .018, .032), gm, bevel=.005))
    for side in (-1, 1):
        objects.append(armored_cowl(f"AX4_REAR_PELVIS_COWL_{side}", (side * .48, .43, 2.72),
                                    (.62, .18, .46), pale,
                                    rotation=(math.pi, side * math.radians(5), side * math.radians(4)), bevel=.024))
    ...
    for side in (-1, 1):
        ...
        for section, z0 in (("THIGH", 2.02), ("SHIN", .98)):
            objects.append(wedge(f"AX4_{p}_{section}_SERVICE_TILE",
                                 (side * .74, -.465, z0), (.16, .030, .13), gm,
                                 front_scale=(.42, .48), rotation=(0, 0, side * math.radians(3)), bevel=.008))
            objects.append(box(f"AX4_{p}_{section}_SEAM",
                               (side * .74, -.487, z0), (.075, .014, .016), dark, bevel=.003))
        for section, z0 in (("BICEP", 3.78), ("FOREARM", 2.96)):
            objects.append(wedge(f"AX4_{p}_{section}_SERVICE_TILE",
                                 (side * 1.60, -.43, z0 + .10),
                                 (.22, .030, .15), gm, front_scale=(.44, .50), bevel=.009))
        for rib in range(4):
...
def add_mir_hero_v3(objects: list[bpy.types.Object], mats: dict[str, bpy.types.Material], rng: random.Random) -> None:
    ...
    for side in (-1, 1):
        ...
        hip = (side * .52, .03, 2.53)
        # Compact armored hoof: the cowl carries load through the ankle while
        # short split toes and a heel talon remain visibly articulated.
        objects.append(armored_cowl(f"M2_{p}_HOOF_BODY", (side * .64, stance_y - .08, .25),
                                    (.54, .48, .34), ivory,
                                    rotation=(math.radians(4), side * math.radians(4), 0), bevel=.022))
        for toe in (-1, 1):
            toe_x = side * .64 + toe * .105
            objects.append(wedge(f"M2_{p}_TOE_PAD_{toe}", (toe_x, stance_y - .23, .105),
                                 (.18, .30, .14), dark, front_scale=(.46, .38), bevel=.014))
            objects.append(sacred_blade_plate(f"M2_{p}_TOE_BLADE_{toe}",
                                              (toe_x, stance_y - .34 + abs(toe) * .01, .16),
                                              (.18, .14, .46), ivory,
                                              rotation=(math.radians(90), 0, toe * math.radians(4)),
                                              sweep=toe * .18, bevel=.012))
        objects.append(wedge(f"M2_{p}_HEEL_PAD", (side * .64, stance_y + .15, .105),
                             (.32, .24, .14), dark, front_scale=(.56, .46), bevel=.014))
        objects.append(sacred_blade_plate(f"M2_{p}_HEEL_SPUR", (side * .64, stance_y + .27, .23),
                                          (.24, .16, .38), rose,
    ...
    for side in (-1, 1):
        ...
        objects.append(sacred_blade_plate(f"M2_WAIST_GUSSET_{side}", (side * .23, -.26, 3.14),
                                          (.32, .22, .58), ivory,
                                          rotation=(math.radians(2), side * math.radians(12), side * math.radians(-7)),
                                          sweep=-side * .18, bevel=.020))
    ...
    for layer, z in enumerate((3.18, 3.43, 3.68)):
        for side in (-1, 1):
            objects.append(sacred_blade_plate(f"M2_V_SHELL_{layer}_{side}",
                                              (side * (.17 + layer * .025), -.43 - layer * .018, z),
                                              (.32 - layer * .020, .17, .48 - layer * .025),
                                              ivory if layer != 1 else rose,
                                              rotation=(math.radians(2), side * math.radians(9), side * math.radians(-16)),
                                              sweep=-side * (.15 + layer * .020), bevel=.016))

    # Six overlapping chest petals define the sacred-machine silhouette.
    chest_specs = [
        (-.42, 4.10, -.18, .60, .96, .31), (.42, 4.10, .18, .60, .96, .31),
        (-.26, 3.78, -.10, .48, .78, .27), (.26, 3.78, .10, .48, .78, .27),
        (-.14, 4.43, -.12, .34, .64, .23), (.14, 4.43, .12, .34, .64, .23),
    ]
