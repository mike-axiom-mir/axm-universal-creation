...
The separate evidence-bound gap compiler can now produce two narrow classes of forge proposal from a real unroutable request: an exact UTF-8 file-route adapter backed by one observed compatible live primitive, or a receipt-discovered project-producer composite backed by one supported exact template/organ producer, the exact verifier, and a closed receipt-to-digest transform. It embeds its gap analysis and runs a request-shaped fixture while leaving semantic equivalence and general behavior explicitly unproven. See `GAP_SYNTHESIS.md`.
