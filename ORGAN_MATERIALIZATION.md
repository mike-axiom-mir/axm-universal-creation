...
## Connect one organ to one creation gap

The `creation-organ-growth` route adds a request-shaped bridge without changing those state-transition rules:

`creation request -> one HOLD_MISSING_ORGAN_INTERFACE -> exact anatomy record + explicit package -> detached Forge test -> ephemeral full assembly`

```bash
PYTHONPATH=src python -m axm_uc create examples/requests/grow_status_panel_creation.json
```

The bridge requires exactly one missing interface and exactly one explicitly supplied package that provides it. It checks the selected anatomy ID against the 415-record census, reuses the materialization compiler, and makes the candidate visible only inside a disposable executable-organ overlay. The final assembly must select the candidate and satisfy the original project shape.

Even a passing bridge result is not adoption. The live installed library, capability routes, source tree, and canonical registry remain unchanged.
